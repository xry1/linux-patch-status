...
本机配置中 `backlog_enabled: true` 开启后台分析。默认每轮最多 2 次、每天最多 30 次 GLM 调用（独立于新邮件摘要的限额）；只分析变化的讨论并缓存结果，单项错误最多尝试三次，失败仍消耗限额。首次积压较多时会跨天逐步完成；重新分析也受限额约束。调用时省略引用和 diff，并限制上下文及输出长度；草稿必须核对事实后自行发送。
