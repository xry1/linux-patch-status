...
AI 摘要通过 loliapi 网关调用 DeepSeek 的 `deepseek-flash`，也兼容其他 Chat Completions / Messages 服务商。配置界面可修改 API 地址、模型和服务商 Key；当前 loliapi 配置继续使用 `PATCH_GLM_API_KEY`，不会把 Key 发往 DeepSeek 官方域名。

本地网页新增“邮箱提醒与 AI 摘要”入口。新邮箱邮件及 AI 草稿仅保存在本机，GitHub Pages 仍由公开 Lore 归档更新；发布脚本会拒绝把私人邮箱报告发布出去。该功能尚需用户配置客户端授权码、所选 AI 服务商 API Key 和飞书机器人后才能实际联网运行。
